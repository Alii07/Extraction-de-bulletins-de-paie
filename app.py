...
if uploaded_pdf is not None and uploaded_file_1 is not None and uploaded_file_2 is not None:
    ...
    def extract_page_number(filename):
        # Utilisation d'une expression régulière pour extraire le numéro après "table_page_"
        match = re.search(r'table_page_(\d+)', filename)
        return int(match.group(1)) if match else 0
    def merge_csv_in_memory(csv_files):
        combined_headers = []
        combined_data = []

        csv_files_sorted = sorted(csv_files.items(), key=lambda x: extract_page_number(x[0]))
        ...
        for i, (filename, csv_content) in enumerate(csv_files_sorted):
            ...
            if i == 0:
                ...
                # Ajouter les nouveaux en-têtes si nécessaire
                for header in headers:
                ...
                for row in rows:
                    new_row = []
                    # Créer un dictionnaire pour map les headers aux valeurs
                    row_dict = {header: value for header, value in zip(headers, row)}
                    for header in combined_headers:
                        new_row.append(row_dict.get(header, ''))  # Ajouter les valeurs ou '' si l'en-tête est manquant
